/* KallistiOS ##version##
   ...
   Test DNS lookup util. This uses raw UDP sockets and not the libc. This
   code will be put into the lwIP port when it's ready, and this example
...
// Use these for testing on a *nix box
...
int dns_make_query(const char * host, dnsmsg_t * buf) {
	int i, o, ls, t;
...
// returned.
int dns_skip_label(dnsmsg_t * resp, int o) {
	int cnt;
...
int dns_copy_label(dnsmsg_t * resp, int o, char * outbuf) {
	int cnt;
	int rv = -1;
...
int dns_parse_response(dnsmsg_t * resp, uint32 * ipaddrout) {
	int i, o, arecs;
	uint16 flags;
	char tmp[64];
...
int main(int argc, char **argv) {
	...
	/*if (argc < 2) {
		printf("specify a name to look up on the cmdline\n");
		return 0;
	} */

	// Setup a query
	size = dns_make_query("www.allusion.net", (dnsmsg_t *)qb);
	for (i=0; i<size; i++) {
		if (i && !(i % 16))
			printf("\n");
		printf("%02x ", qb[i]);
	...
	toaddr.sin_addr.s_addr = htonl(0x0a030202);	// Put your DNS server address here
	...
	if ((size = recvfrom(sock, qb, 1500, 0, (struct sockaddr *)&toaddr, &addrsize)) < 0) {
		perror("can't recv from server");
		return -1;
	}
	// KOS doesn't fill these in correctly yet
	/*assert( toaddr.sin_port = htons(53) );
	assert( toaddr.sin_addr.s_addr = htonl(0x0a030202) ); */
	...
	// Parse the response
	dns_parse_response((dnsmsg_t *)qb, &ip);
